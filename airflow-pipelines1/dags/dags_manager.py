...
# Когда по отдельности собираю докерфайлы с помощью команды --network=host, то зависимости скачиваются
# Возможно какая-та сетевая проблема или в настройках docker-compose.
# Попробовал собрать локальнь, залить на докер заб и от туда скачать, но теперь не скачиваются имейджи.
    load_data >> predict
